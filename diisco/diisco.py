...
from diisco.prior_w_model import PriorWModel
from diisco import utils
...
class DIISCO:
    ...
    def __init__(
        ...
        # EA: ADDED
        self.prior_w_mean = None  # The means to use for the emission model
        self.prior_w_cov = (
            None  # The covariance to use for the variational distribution
        )
    ...
    def fit(
        ...
        # EA: ADDED
        self.fit_and_set_w_prior_params(
            timepoints=timepoints,
            levels=levels,
            hypers=self.hypers_init_vals,
        )
    ...
    def _model(self, timepoints, proportions=None):
        ...
        """
        # EA: ADDED
        w_mean = self.prior_w_mean
        w_covariance = self.prior_w_cov
        #
        ...
        with pyro.plate("cell_types_outer_W", n_cell_types, dim=-2):
            with pyro.plate("cell_types_inner_W", n_cell_types, dim=-1):
                assert w_mean.shape == (n_cell_types, n_cell_types, n_timepoints, 1)
                assert w_covariance.shape == (
                    n_cell_types,
                    n_cell_types,
                    n_timepoints,
                    n_timepoints,
                ...
                W = pyro.sample(
                    names.W,
                    dist.MultivariateNormal(w_mean, w_covariance),
                    infer=dict(
                        baseline={
                            "use_decaying_avg_baseline": True,
                            "baseline_beta": 0.95,
    ...
    def fit_and_set_f_prior_params(
        ...
        print("Set f prior parameters: mean " + str(self.prior_f_mean) + ", cov " + str(self.prior_f_cov))
    ...
    def _get_guide(self, guide_name: str, model: Callable) -> Callable:
        ...
        if guide_name == "MultivariateNormal":
            ...
            return guides.MultivariateNormalFactorized(
                self.n_cell_types, self.n_timepoints, self.prior_f_mean, self.prior_w_mean
            )
        else:
            raise ValueError(
    ...
    def fit_and_set_w_prior_params(
            self,
            timepoints: torch.Tensor,
            levels: torch.Tensor,
            hypers: dict,
            ...
            """
            Set the prior for the model for w by fitting a GP to the
            protein levels.

            :param timepoints: Tensor of timepoints for the samples.
                shape: (n_timepoints, 1)
            :param levels: Tensor of levels for the samples.
                shape: (n_timepoints, n_cell_types, n_cell_types)
            """
            length_scale = hypers[names.LENGTHSCALE_W]
            variance = hypers[names.VARIANCE_W]
            sigma_w = hypers[names.SIGMA_W]
            
            prior_w_model = PriorWModel(length_scale=length_scale, variance=variance, sigma_y=sigma_w)
            prior_w_model.fit(timepoints, levels)
            self.prior_w_mean, self.prior_w_cov = prior_w_model.predict(timepoints)
            print("Set W prior parameters: mean " + str(self.prior_w_mean) + ", cov " + str(self.prior_w_cov))
            self._prior_is_set = True